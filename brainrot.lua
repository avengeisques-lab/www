...
local function updateESP()
    ...
    if brains then
        for _, brain in pairs(brains:GetChildren()) do
            if brain:IsA("BasePart") then
                createESP(brain, config.espColor, "Brain")
            end
        end
    end
    ...
    if bases then
        for _, base in pairs(bases:GetChildren()) do
            if base:IsA("BasePart") or base:IsA("Model") then
                local primaryPart = base:IsA("Model") and base.PrimaryPart or base:FindFirstChildWhichIsA("BasePart")
                if primaryPart then
                    createESP(primaryPart, config.baseEspColor, "Base")
                end
            end
        end
    end
    ...
    if map then
        for _, obj in pairs(map:GetDescendants()) do
            if obj:IsA("BasePart") and obj.Name:lower():match("base") then
